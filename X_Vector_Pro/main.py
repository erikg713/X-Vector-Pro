import os, time, socket, json, random from queue import Queue from datetime import datetime from tkinter import messagebox, filedialog import customtkinter as ctk import requests, xmlrpc.client, tldextract import threading from urllib.parse import urljoin from PIL import Image

from utils.toast import ToastManager from utils.stealth import enable_stealth_mode from utils.logger import log_encrypted from gui.tabs.auto_tab import AutoTab from gui.tabs.brute_tab import BruteTab from gui.tabs.recon_tab import ReconTab from gui.tabs.exploit_tab import ExploitTab from gui.tabs.logs_tab import LogsTab from gui.tabs.settings_tab import SettingsTab
...
def log_to_file(module, content): os.makedirs("logs", exist_ok=True) timestamp = datetime.now().strftime("%Y%m%d_%H%M%S") with open(f"logs/{module}_{timestamp}.log", "a") as f: f.write(content + "\n")

def show_toast(msg, level="info"): color = {"info": "#2fa4ff", "error": "#ff5252", "success": "#27c93f"}.get(level, "#333") toast = ctk.CTkToplevel() toast.geometry("250x50+1050+80") toast.configure(bg=color) toast.wm_overrideredirect(True) label = ctk.CTkLabel(toast, text=msg, fg_color=color, text_color="white") label.pack(fill="both", expand=True) toast.after(3000, toast.destroy)

def stealth_delay(min_ms=200, max_ms=800): time.sleep(random.uniform(min_ms / 1000, max_ms / 1000))

def run_dir_scan(): url = scanner_target_entry.get().strip().rstrip("/") if not url.startswith("http"): show_toast("Use full URL (http/https).", "error") return
